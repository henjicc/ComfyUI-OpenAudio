...
This repository includes a ComfyUI custom node for OpenAudio S1-mini model inference. To use it:

1. Make sure you have installed the OpenAudio project dependencies
2. Copy this folder to your ComfyUI custom_nodes directory
3. Restart ComfyUI
4. You will find the "OpenAudio" node in the node menu under the "OpenAudio" category

The node supports:
- Text-to-speech conversion using the S1-mini model
- Voice cloning with reference audio (optional)
- Adjustable parameters like temperature, top-p, and repetition penalty

Load the `test_workflow.json` in ComfyUI to see a basic example.

### Model Directory Support

The OpenAudio node automatically detects and uses ComfyUI's model directory structure. It will look for models in:
- `ComfyUI/models/OpenAudio/openaudio-s1-mini/` (recommended)
- Local `checkpoints/openaudio-s1-mini/` directory as fallback

Model files needed:
- LLaMA model files in `openaudio-s1-mini` directory
- Decoder model file at `openaudio-s1-mini/codec.pth`

### Automatic Model Download

The new model loader node can automatically download the OpenAudio S1-mini model if it's not found locally:

1. Make sure you have `git` installed and available in your system PATH
2. Enable the "auto_download" option in the OpenAudio Model Loader node
3. The model will be automatically downloaded from HuggingFace when needed using `git clone`

Model download URL: https://huggingface.co/fishaudio/openaudio-s1-mini

### New Model Loader Node

We've added a dedicated model loader node (`OpenAudio Model Loader`) that handles model loading and management. This node:
- Loads the OpenAudio S1-mini model
- Manages device selection (CPU/GPU)
- Automatically downloads models if not found locally
- Provides the model object to other nodes

Use `test_model_workflow.json` to see how to use the new model loader node.

### Dependencies

For ComfyUI usage, you only need to install the essential dependencies:

```bash
pip install -r requirements.txt
```

This requirements.txt file contains only the necessary dependencies for running the OpenAudio node in ComfyUI, excluding many dependencies from the full project that are not needed in the ComfyUI environment.